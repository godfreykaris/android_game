...
namespace objl
{
	...
	public:
		...
		//get a line from the  file using the android asset manager and return it as a string 
		std::string curline;
		FILE* modelfile;

		bool mline(FILE* file)
		{
			curline.clear();
			char mstring[100];

			fgets(mstring, 100, file);
						
			curline.append(mstring);
		...
		bool LoadFile(std::string Path)
		{
			...
			modelfile = android_fopen(Path.c_str(), "rb");
			
			
			/*for (int i = 0; i < 3; i++)
			{
				mline(curline, modelfile);
			}*/
			

			while (mline(modelfile) == 0/*std::getline(file, curline)*/)
			{
	...
	private:
		...
		bool LoadMaterials(std::string path)
		{
			...
			if (path.substr(path.size() - 5, path.size()) != ".mtl\n")
				return false;
			std::string mpath = path.substr(0, path.size() - 1);
			FILE* materialf = android_fopen(mpath.c_str(), "rb");
